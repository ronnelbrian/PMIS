...
function loadtable()
{
  ...
  $('#spmsTable').dataTable({
      "serverSide": true,
      "sAjaxSource": "<?php echo base_url('Process/approve_job_request')?>"+"?loadtable=true",
      "deferLoading": 10,
      "bPaginate": true,
...
function approve_(id, name){
  ...
  if (result) {
       $.ajax
        ({ type:"POST",
           async: true,
           url:"<?php echo base_url('Process/approve_job_request')?>",
