<div id="page-content">
  <div id='wrap'>
    <div id="page-heading">
      <ol class="breadcrumb">
        <li>Administrator</li>
          <li>Furniture / Equipment Order</li>
      </ol>
    </div>
    <div class="container">
      <div class="row">
        <div class="col-md-12">
          <div class="panel panel-inverse">
              <div class="panel-heading">
                  <h4>Furniture / Equipment Order Form</h4>
                  <div class="options">
                    <a class="panel-collapse" href="#">
                      <i class="fa fa-chevron-up"></i></a>
                  </div>
              </div>
              <form action="" class="form-horizontal row-border"  method="POST" id="furniture_order_form">
              <input type="hidden" name="add_furniture_order" id="add_furniture_order"/>
                      ...
                      <div id="searchResult">
                        ...
                        <div class="col-md-12 group" ><br>
                          ...
                          <div class="form-group" id="row_1">
                            <div class="col-md-3">
                              <select class="form-control"  id="item" name="item[]" style="font-size:12px;width:100%"  required>
                            ...
                            <div class="col-md-2" id="" >
                              <input type="number" class="form-control" name="price[]" id="price"  style="font-size:12px; text-align:right"  placeholder="0.00" min = "0" required>
                            </div>
                            <div class="col-md-2" id="" >
                   ...
                   <form action="" class="form-horizontal row-border"  method="POST" id="searchForm">
                      <input name="_table" id="_table" type="hidden"/>
                        ...
                        <div class="form-group" style="padding:0px">
                          <label class="col-sm-2 control-label"><b>F.O. #</b></label>
                          <div class="col-sm-4">
...
function loadtable()
{
  ...
  $('#spmsTable').dataTable({
      "serverSide": true,
      "sAjaxSource": "<?php echo base_url('Process/furniture_and_equipment_order')?>"+"?loadtable=true&"+$('#searchForm').serialize(),
      "deferLoading": 10,
      "bPaginate": true,
      "aaSorting": [[0,'desc']],
...
function cancel_(id, name){
  ...
  if (result) {
       $.ajax
        ({ type:"POST",
           async: true,
           url:"<?php echo base_url('Process/furniture_and_equipment_order')?>",
           ...
           success: function(data)
            {
              if (data['mes'] == "Success")
              {
                alerts('Furniture / Equiptment Order has been cancelled.','s');
...
function view_(id, name, status)
{
  ...
  {
    var result = confirm('This Furniture/Equiptment Order ('+name+') will now be tagged released. Do you want to proceed?');
    if(result)
        window.open('<?php echo base_url("Process/view_fo")?>'+'?id='+id);
    ($('#_table').val() == "1")?loadtable():null;
  }
  else
    window.open('<?php echo base_url("Process/view_fo")?>'+'?id='+id);
...
$(document).ready(function(){
  ...
  $("#furniture_order_form").on('submit',(function(e){
      e.preventDefault();
