...
class savedata_model extends CI_Model{
	...
	public function notifApprover($id, $requested_by = 0)
	{
		...
		$d = $this->db->query("SELECT CONCAT(ril.unit,' ',u.code,' ',p.description) details FROM request_item_line ril 
	...
	public function product($input = array())
	{
		...
		{
			$data = array('description' => $description,'category_id' => $category_id, 'critical_level' => $critical_level, 'unit_price' => $unit_price,'measurement' => $measurement,  'updated_by' => $this->session->userdata('USERID'), 'date_updated' => date('Y-m-d h:i:s'),'types'=>$type);
			$this->db->where('id', $id);
	...
	public function purchase_order($input = array())
	{
		if($input['act'] == 'add')
		{
			...
			$price = $this->security->xss_clean($input['requested_unit_price']);
			$unit = $this->security->xss_clean($input['requested_qty']);
			$discount = $this->security->xss_clean($input['requested_discount']);
			$amount = $this->security->xss_clean($input['requested_total_amount']);

			$item = $this->security->xss_clean($input['requested_item']);
			...
			// 	($discount[$i] == '')?$discount[$i] = 0:null;
				$data = array(
					...
					'po_id' => $poid,
					'discount' => $discount,
					// 'amount' => $price*$unit-($price*(($discount*$unit)/100)));
					'amount' => $amount);
...
*/
	...
	public function notifApproverPR($id, $requested_by = 0)
	{
		$q = $this->db->query("SELECT * from (SELECT CONCAT(u.lname, ', ', u.fname) NAME, u.id,
									...
									where tbl.approval_status = 2", $id);
		$d = $this->db->query("SELECT CONCAT(ril.unit,' ',u.code,' ',p.description) details FROM purchase_request_line ril 
									INNER JOIN product p 
									ON p.id = ril.product
									INNER JOIN units u 
									ON u.id = p.measurement
	...
	public function purchase_request_to_po($input = array()){
		...
		//	$requested_item_discount = $this->security->xss_clean($input['requested_item_discount']);
